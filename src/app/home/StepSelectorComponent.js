...
const StepSelector = ({next, prev, nextStep, prevStep, isReadyToAdvance, results, getResults}) => {
  const prevButton = prev ? <button onClick={(e) => {prevStep(e)}} className="tntool__button tntool__button--back">Back</button> : null
  const nextButton = next ? <button disabled={!isReadyToAdvance} onClick={(e) => {nextStep(e)}} className="tntool__button tntool__button--next">Next</button> : null
