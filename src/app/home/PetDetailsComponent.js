...
const PetDetails = ({styles, petDetails, handleUpdatePetName, petType = "dog", scrollTo}) => {
  return (
    <div style={styles}>
      ...
      <input className="tntool__input tntool__input--name" type="text" name="name" required value={petDetails.name} onFocus={() => scrollTo(".tntool__banner--finder", 1000)} onChange={(e) => handleUpdatePetName(e.target.value)} />
    </div>
  )
}
